...
실시간 매칭·통화 서비스 VoiceLink를 1인 개발하고, Redis·LiveKit·Nginx·Docker 기반 운영 환경까지 직접 구축했습니다. 매칭 취소 직후 과거 결과가 재노출되는 문제와 통화 종료 후 재매칭 시 유령 세션에 진입하는 동시성 이슈를 해결하기 위해, Redis Lua Script 기반 Atomic Claim, Cancel Marker 재검증, DB Outbox + Redis Pub/Sub + TTL result key 결과 전파, 현재 연결 기준 대기열 정리, `CallSession.ended_at` 기준 세션 종료 처리, LiveKit webhook, 참가자 수 기반 활성 세션 검증 로직을 적용했습니다.
