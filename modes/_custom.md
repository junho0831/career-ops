...
2. **외주/프리랜서 절대 원칙 (100% 원격 필수 / 비개발자 맞춤형 포트폴리오 노출 전략)**:
   - 위시켓, 프리모아 등 외주 프로젝트 지원 시 **"상주 필수", "파견", "주 N일 출근" 조건의 프로젝트는 필터링 단계에서 즉시 제외(지원 불가)**한다.
   - 100% 원격/재택 근무가 가능한 프로젝트에만 선별 지원한다.
   - 제안서 작성 시 비개발자 클라이언트(대표님, 사업 담당자)가 단번에 이해할 수 있도록 어려운 기술 용어(MVCC, Lua Script 등)를 실질적 비즈니스 혜택(처리 속도 단축, 서버 다운 방지, 관리자 재처리 편의성 등)으로 번역하여 작성한다.
   - **포트폴리오(`so-dak.com`) 선별 노출 원칙**:
     - **클라이언트가 신뢰/실체(진짜 혼자서 배포/운영해 본 사람인가)를 중시하는 경우 (⭕ 적극 포함)**: "제가 1인으로 설계·배포하여 운영 중인 실시간 서비스(VoiceLink)와 수천만 건 데이터 처리 기록을 보실 수 있는 공식 사이트([https://so-dak.com/](https://so-dak.com/))를 남겨드립니다"처럼 **친절한 한 줄 설명과 함께 첨부**한다.
     - **단순 단기 API 수정이나 문서가 지나치게 기술적이라 비개발자에게 거부감을 줄 수 있는 경우 (❌ 과감히 생략)**: 복잡한 블로그 링크 대신 제안서 본문의 쉬운 문제 해결 사례와 마일스톤에 집중한다.
