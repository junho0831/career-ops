...
3. **채용 포털 맞춤 이력서 선택 원칙 (Company/JD-Tailored CV)**:
   - 원티드, 사람인, 잡코리아 지원 시 회사의 도메인과 JD 요구 역량(대용량 데이터/배치, 실시간 백엔드, Python/SaaS, 정합성/안정성)에 부합하는 버전의 이력서를 선별하여 제출한다.
...
- **"채용 지원"**: 원티드, 사람인, 잡코리아에서 `지원완료` 여부를 체크하고, JD 분석 후 매칭되는 최적의 이력서 버전을 선택하여 원클릭/빠른 입사지원 수행.

## Output Preferences

- 지원 전후 상태(`지원완료`, `제출완료`, `상주 제외`)를 명확하게 1줄 요약으로 보고한다.
